...
`{ size: 12 }` both select 12 points. Zero and `NaN` select the 14pt default as
they do on native, while a negative size throws a `RangeError` naming the option
and the value, because a negative size can only produce nonsensical metrics.
