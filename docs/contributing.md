...
Documentation changes must use Markdown under `docs/` and pass a strict MkDocs
build.

## Local Preview

The Python dependencies in `docs/requirements.txt` are needed only when you
want to preview or edit this documentation. They are not required to install,
build, or use MuhammaraJS.
...
Copyable examples belong in `docs/examples/`. A matching test under
`tests/docs/` must execute each example in a temporary directory and verify its
output. Do not document behavior based only on an untested snippet.
...
```text
https://github.com/julianhille/MuhammaraJS/blob/develop/tests/path/to/test.js
```
...
- Use the package name `muhammara` in JavaScript examples.
