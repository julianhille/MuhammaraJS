...
function makeTextObjects(self, textObject = {}, pathOptions, textBox = {}) {
  ...
  const previousTextObjects = self._previousTextObjects2 ? self._previousTextObjects2 : [];
  let text =
  ...
  if (previousTextObjects.length > 0) {
    let lineWidth = 0;
    let spaceSz = 0;
    const end = previousTextObjects.length - 1;
    const previousLine = previousTextObjects[end];
    ...
    for (let i = end; i >= 0; i--) {
      let textObj = previousTextObjects[i];
  ...
  while (bk) {
    ...
    if (newLine.canFit(word)) {
      ...
      const objects = toWriteTextObjects.length ? toWriteTextObjects : previousTextObjects;
  ...
  if (!flushLine) {
    ...
    self._previousTextObjects2.push(incompleteTextObject);
  } else {
