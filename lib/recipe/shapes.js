...
exports.triangle = function triangle(x, y, traits, options = {}) {
  ...
  if (options.debug) {
    ...
    let [rx, ry] = triopts.rotationOrigin
      ? triopts.rotationOrigin
...
const Triangle = class Triangle {
  ...
  get incenter() {
    if (!this._incenter) {
      ...
      let x =
        (this._a * this._A[0] + this._b * this._B[0] + this._c * this._C[0]) /
        this.perimeter;
      let y =
        (this._a * this._A[1] + this._b * this._B[1] + this._c * this._C[1]) /
        this.perimeter;
...
exports.arrow = function arrow(x, y, options = {}) {
  ...
  if (debug) {
    ...
    if (debug === 2) {
      // Display Kite reference points
      const tc = "red";
      const cc = "red";
      const ktc = "blue";
      const kcc = "green";
      this.text("E", arrow.tip.E[0] - 3, arrow.tip.E[1] - 4, {
        size: 9,
        color: ktc,
      });
      this.circle(arrow.tip.E[0], arrow.tip.E[1], 6, {
        color: kcc,
        width: 0.5,
      });
      this.text("K", arrow.tip.K[0] - 3, arrow.tip.K[1] - 10, {
        size: 9,
        color: ktc,
      });
      this.circle(arrow.tip.K[0], arrow.tip.K[1] - 6, 6, {
        color: kcc,
        width: 0.5,
      });
      this.text("i", arrow.tip.I[0] + 5, arrow.tip.I[1] - 4, {
        size: 9,
        color: ktc,
      });
      this.circle(arrow.tip.I[0] + 6, arrow.tip.I[1], 6, {
        color: kcc,
        width: 0.5,
      });
      this.text("T", arrow.tip.T[0] - 2, arrow.tip.T[1] + 3, {
        size: 9,
        color: ktc,
      });
      this.circle(arrow.tip.T[0], arrow.tip.T[1] + 6, 6, {
        color: kcc,
        width: 0.5,
      });
...
const Line = class Line {
  constructor(x1, y1, x2, y2) {
    ...
    if (
      (x1 instanceof Point || Array.isArray(x1)) &&
      !(y1 instanceof Point || Array.isArray(y1))
    ) {
  ...
  get slope() {
    if (!this._slope) {
      this._slope =
        Math.abs(this._pt2.x - this._pt1.x) < 0.0001
          ? Infinity
          : (this._pt2.y - this._pt1.y) / (this._pt2.x - this._pt1.x);
    }
  ...
  extend(distance, ptNbr = 2) {
    ...
    let direction =
      Math.abs(newLen - (this.length + distance)) < epsilon ? 1 : -1;
    x += direction * deltaX;
    y += direction * deltaY;
