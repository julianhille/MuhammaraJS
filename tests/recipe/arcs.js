...
function pie(x, y, radius, chart) {
  ...
  if (chart.sector) {
    if (chart.sector.stroke) {
      sectorLineColor = chart.sector.stroke;
    }
    if (chart.sector.opacity) {
      sectorOpacity = chart.sector.opacity;
    }
    if (chart.sector.ray) {
      sectorRay = chart.sector.ray;
    }
    if (chart.sector.offset) {
      sectorOffset = chart.sector.offset;
    }
    if (chart.sector.percentColor) {
      sectorPcntColor = chart.sector.percentColor;
    }
    if (chart.sector.percentSize) {
      sectorPcntSize = chart.sector.percentSize;
    }
    if (chart.sector.percentLimit) {
      sectorLimit = chart.sector.percentLimit;
    }
  }
  ...
  for (let i = 0; i < pie.length; i++) {
    ...
    if (pie[i].stroke) {
      sectorOpts.stroke = pie[i].stroke;
    }
    if (pie[i].width) {
      sectorOpts.width = pie[i].width;
    }
    if (pie[i].opacity) {
      sectorOpts.opacity = pie[i].opacity;
    }
    ...
    if (sectorSize < sectorLimit) {
      ...
      this.text(percent, px + tx, py + ty, {
        color: sectorPcntColor,
        size: pcntSize,
      });
    }
    ...
    if (pie[i].label) {
      ...
      if (!opts.color) {
        opts.color = sectorColors[i];
      }
      let xOffset = 5;
      let yOffset = opts.size / 2;
      const label = needPercent
        ? `${pie[i].label} (${sectorSize}%)`
        : pie[i].label;

      if (sectorRay) {
        this.line([rayPt1, rayPt2], { stroke: sectorColors[i], width: 0.5 });
      }
...
describe("Arc test", () => {
  it("Simple Arcs", (done) => {
    ...
    recipe
      ...
      .arc(x, y, r, 0, 90)
      .text("0°", x + r + 5, y, txtOps)
      .text("90°", x, y + r + 5, txtOps)
      .arc(x, y, r, -90, -180)
      .text("-90°", x - 15, y - r - 13, txtOps)
      .text("-180°", x - r - 30, y - 10, txtOps)
      .circle(x, y, 2, { color: "red", width: 0.5 })
      .arc(x + 120, y, r, 90, -90)
