...
describe("Color", () => {
  it("basic", (done) => {
    ...
    pageContext.drawCircle(149, 640, 40, {
      color: 0x0000ff00,
      colorspace: "cmyk",
      type: "fill",
    });
    ...
    pageContext.drawPath(75, 640, 149, 800, 225, 640, {
      color: 0xff000000,
      colorspace: "cmyk",
      type: "fill",
    });
    pageContext.drawPath(75, 700, 149, 750, 225, 700, {
      color: 0xbb,
      colorspace: "gray",
      type: "stroke",
      width: 5,
    });
    pageContext.drawCircle(149, 590, 50, {
      color: 0xff0000,
      colorspace: "rgb",
      type: "fill",
    });
  ...
  it("All color spaces", (done) => {
    ...
    recipe
      ...
      .text("C", gsize + cwid * 4 - 10, line(i), {
        size: title,
        color: [255, 0, 0, 0],
      })
      .text("M", gsize + cwid * 5 - 10, line(i), {
        size: title,
        color: [0, 255, 0, 0],
      })
      .text("Y", gsize + cwid * 6 - 6, line(i), {
        size: title,
        color: [0, 0, 255, 0],
      })
      .text("K", gsize + cwid * 7 - 10, line(i), {
        size: title,
        color: [0, 0, 0, 255],
      });
    i += 1;
    recipe
      ...
      .text("Gray, PercentColor: %50", para, line(++i), { color: "%50" })
      .text("Gray, HexColor: #55", para, line(++i), { color: "#55" })
      .text("RGB,  DecimalColor: [255,0,0]", para, line(++i), {
        color: [255, 0, 0],
      })
      .text("RGB,  PercentColor: %0,100,0", para, line(++i), {
        color: "%0,100,0",
      })
      .text("RGB,  HexColor: #0000FF", para, line(++i), { color: "#0000FF" })
      .text("CMYK, DecimalColor: [255,0,0,0]", para, line(++i), {
        color: [255, 0, 0, 0],
      })
      .text("CMYK, PercentColor: %0,28,100,6", para, line(++i), {
        color: "%0,28,100,6",
      })
      .text("CMYK, HexColor: #FF000088", para, line(++i), {
        color: "#FF000088",
      });
    i += 1;
    recipe
      .text("Colorspace Defaults", col, line(++i))
      .circle(icon, line(i + 1) + 8, 5, { colorspace: "gray", fill: "#ffff" })
      .text("Gray", para, line(++i), { colorspace: "gray" })
      .circle(icon, line(i + 1) + 8, 5, { colorspace: "rgb", fill: "#00" })
      .text("RGB", para, line(++i), { colorspace: "rgb" })
      .circle(icon, line(i + 1) + 8, 5, { colorspace: "cmyk", fill: "#00" })
      .text("CMYK", para, line(++i), { colorspace: "cmyk" });
    i += 1;
    recipe
      .text('Bad "color" value results in colorspace defaults', col, line(++i))
      .circle(icon, line(i + 1) + 8, 5, { colorspace: "gray", fill: "#ffff" })
      .text("Gray, #ffff", para, line(++i), {
        colorspace: "gray",
        color: "#ffff",
      })
      .circle(icon, line(i + 1) + 8, 5, { colorspace: "rgb", fill: "#00" })
